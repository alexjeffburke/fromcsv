...
class IgnoredColumn extends Column {}
class UnmatchedColumn extends Column {}
...
module.exports = function FromCsv(options) {
  ...
  function columnsCountNull(arr) {
    return arr.map(flattenColumn).filter(isNull).length;
  }
  ...
  function isBetterHeaderMatch(contender, original) {
    return columnsCountNull(contender) < columnsCountNull(original);
  }
  ...
  function matchHeaderByHash(matchHash, headerRow) {
    ...
    headerRow.forEach(function(columnName, columnIndex) {
      ...
      } else if (typeof columnName === "object" && columnName) {
        matchedNames.push(columnName);
        unmatchedNamesByIndex[columnIndex] = columnName;
      } else {
        columnName = new UnmatchedColumn(columnName);
        matchedNames.push(columnName);
        unmatchedNamesByIndex[columnIndex] = columnName;
      }
    });
  ...
  function processRow(csvConfig, headerInfo, dataRow) {
    ...
    dataRow.forEach(function(value, columnIndex) {
      ...
      const isIgnored = columnName instanceof IgnoredColumn;
      let isUnmapped = isIgnored;
      if (columnName instanceof UnmatchedColumn) {
        isUnmapped = true;
  ...
  function processCsvData(headerInfo, csvRows, forceImport = false) {
    ...
    const flattedColumns = columns.map(flattenColumn);
    const flattenedMatchedColumns = headerInfo.matchedColumns.map(
      flattenColumn
    );
    ...
    return {
      ...
      columns: {
        ...
        present: flattedColumns,
        missing: _.difference(flattenedMatchedColumns, flattedColumns),
        unmatched: Object.values(headerInfo.unmatchedColumnsByIndex).map(
          column => extractColumn(column)
        )
      },
      rows: rows
    };
  ...
  this.fromData = function fromData(inputData, forceImport = false) {
    ...
    const processedHeader = header.map(h =>
      h === null ? new IgnoredColumn(null) : h
    );
    var headerInfo = attemptMatchHeader(processedHeader);
